...
// TODO: add @codewars/test-compat support
async function scrapeJavascript()
{
    await client.connect();
    const db = client.db("CodingChallengeBot");
    const collection:Collection = db.collection('Challenges');
    ...
    page.on('response', async response => {
        if (response.url().includes("/javascript/session")) {
            var r = await response.json()
            //@ts-ignore
            var doc = (await collection.find({"_id": currentDoc}).toArray())[0]
            if (!doc.code)
            ...
            doc.code["javascript"] = {setup: r.setup, exampleFixture: r.exampleFixture}
            //@ts-ignore
            await collection.replaceOne({"_id": currentDoc}, doc)
            waiting = false
        }
    ...
    var allDocs = await collection.find({"languages": {"$in": ["javascript"]}}).toArray()
    ...
    for (let index = 0; index < allDocs.length; index++) {
        ...
        console.log("fetching: " + currentDoc + " index: " + index + ":" + allDocs.length)
        waiting = true
        await page.goto(`https://www.codewars.com/kata/${currentDoc}/train/javascript`);
        while (waiting)
        {
            await delay(100)
        }
